...
/**
 * Use case for fetching recipes filtered by a specific tag.
...
class GetRecipesByTagUseCase @Inject constructor(
    ...
    /**
     * Fetches recipes filtered by tag and emits loading, success, or error states.
     *
    ...
    operator fun invoke(
        ...
        try {
            val apiResponse: ApiResponse<List<Recipe>> = recipeRepository.getRecipesByTag(
                tag = tag,
                sortBy = sortBy,
                order = order
            )
